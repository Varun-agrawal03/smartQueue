...
const EventDetailPage = () => {
  ...
  const [bookingStatus, setBookingStatus] = useState<"idle" | "queued" | "success" | "error">("idle");
  ...
  useSocket(id || null, {
    ...
    onSeatUpdated: (data) => {
      setSeats((prev) =>
        prev.map((s) =>
          s.id === data.seatId ? { ...s, status: data.status as Seat["status"] } : s
        )
      );
      // Deselect if someone else booked the seat you selected
      if (selectedSeat?.id === data.seatId) {
        setSelectedSeat(null);
        setBookingMessage("⚠️ This seat was just booked by someone else!");
      }
    },
  });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setCurrentPrice(eventRes.data.event.current_price || eventRes.data.event.base_price);
      ...
      const poll = setInterval(async () => {
        try {
          ...
          if (state === "completed" && result) {
            ...
            setBookingMessage(`✅ Seat ${selectedSeat.seat_number} booked for ₹${Number(result.price_paid).toLocaleString()}!`);
            setSelectedSeat(null);
            setBooking(false);
            ...
            setBookingMessage(
              failReason?.includes("SEAT_LOCKED") ? "❌ Seat is temporarily locked. Try another." :
              failReason?.includes("SEAT_UNAVAILABLE") ? "❌ Seat already booked." :
  ...
  const getSeatColor = (seat: Seat) => {
    if (seat.status === "booked") return "bg-red-800 cursor-not-allowed opacity-60";
    if (seat.status === "locked") return "bg-yellow-700 cursor-not-allowed opacity-60";
    if (selectedSeat?.id === seat.id) return "bg-indigo-500 ring-2 ring-white scale-110";
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-gray-900 rounded-2xl p-6 border border-gray-800">
            ...
            <p className="text-gray-500 text-sm mb-4">📅 {formatDate(event.event_date)}</p>
            <p className="text-gray-400 text-sm">{event.description}</p>
          </div>

          {/* Seat map */}
          <div className="bg-gray-900 rounded-2xl p-6 border border-gray-800">
            <h2 className="text-xl font-bold text-white mb-6">Select Your Seat</h2>
            ...
            <div className="flex gap-6 mt-8 pt-4 border-t border-gray-800">
              ...
              <div className="flex items-center gap-2 text-sm text-gray-400">
                <div className="w-4 h-4 rounded bg-red-800 opacity-60"></div> Booked
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="bg-gray-900 rounded-2xl p-6 border border-gray-800">
            ...
            {selectedSeat ? (
              <div className="space-y-4">
                <div className="bg-indigo-900/30 border border-indigo-700 rounded-xl p-4">
                  <div className="text-indigo-300 text-sm">Selected Seat</div>
                  <div className="text-white text-2xl font-bold">
                    {selectedSeat.seat_number}
                  </div>
                  <div className="text-indigo-400 text-sm">Row {selectedSeat.row_label}</div>
                </div>
            ...
            {bookingMessage && (
              <div className={`mt-4 p-3 rounded-xl text-sm text-center ${
                bookingStatus === "success" ? "bg-green-900/30 text-green-400 border border-green-800" :
                bookingStatus === "error" ? "bg-red-900/30 text-red-400 border border-red-800" :
